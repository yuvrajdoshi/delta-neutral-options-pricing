...
class RealWorldBacktestEngine:
    ...
    def run_backtest(self, start_date: str, end_date: str, 
        ...
        """
        print(f"🎯 REAL-WORLD BACKTEST: {symbol}")
        ...
        if test_volatility_threshold is not None:
            ...
            print(f"   Using normal threshold: {original_threshold:.1%}")
        
        # Get market data
        data_manager = MarketDataManager()
        dataset = data_manager.get_comprehensive_dataset(symbol, start_date, end_date)
        
        if dataset.empty:
            print("❌ No market data available for backtest period")
        ...
        self.strategy.portfolio_value = self.initial_capital
        
        # Run day-by-day simulation
        print(f"\n📈 Running daily simulation...")
        
        for i in range(self.strategy.lookback_period, len(dataset)):
            current_date = dataset.index[i]
            
            # Get data up to current date for signal generation
            historical_data = dataset.iloc[:i+1]
            
            # Store positions before update to track closures
            positions_before = {pos_id: position for pos_id, position in self.strategy.positions.items() if position.is_active}
            
            # Update existing positions
            self.strategy.update_positions(historical_data)
            
            # Check for closed positions and add to results
            for pos_id, position in positions_before.items():
                if pos_id in self.strategy.positions:
                    current_position = self.strategy.positions[pos_id]
                    if not current_position.is_active:
                        # Position was closed, add to results
                        results.add_trade(current_position, current_position.pnl)
                        if current_position.pnl > 0:
                            results.winning_trades += 1
                        else:
                            results.losing_trades += 1
                        results.total_pnl += current_position.pnl
            
            # Generate new signal
            signal = self.strategy.generate_signals(historical_data)
            
            if signal:
                # Execute signal with costs
                position = self.execute_signal_with_costs(signal)
                if position:
                    results.signals_executed += 1
                
                results.signals_generated += 1
            
            # Record daily portfolio value and return
            results.daily_portfolio_values.append(self.strategy.portfolio_value)
            
            if len(results.daily_portfolio_values) > 1:
                daily_return = (self.strategy.portfolio_value - results.daily_portfolio_values[-2]) / results.daily_portfolio_values[-2]
                results.daily_returns.append(daily_return)
        
        # Handle any remaining open positions at the end
        for pos_id, position in self.strategy.positions.items():
            if position.is_active:
                # Close remaining positions and add to results
                results.add_trade(position, position.pnl)
                if position.pnl > 0:
                    results.winning_trades += 1
                else:
                    results.losing_trades += 1
                results.total_pnl += position.pnl
        
        # Restore original threshold
        self.strategy.entry_threshold = original_threshold
        
        # Set final portfolio value on results
        results.final_portfolio_value = self.strategy.portfolio_value
        
        print(f"✅ Backtest completed!")
        print(f"   Signals generated: {results.signals_generated}")
        print(f"   Trades executed: {results.signals_executed}")
        print(f"   Final portfolio value: ${self.strategy.portfolio_value:,.2f}")
        
        return results

    def execute_signal_with_costs(self, signal: Signal) -> Optional[Position]:
        """
